...
void actionWindowSizeTriple() {
	state.windowedSize = P_VIDEO_SIZEX3;
	if (state.properties->video.windowSize != P_VIDEO_SIZEX3) {
		state.properties->video.windowSize = P_VIDEO_SIZEX3;
		state.properties->video.windowSizeChanged = 1;
		archUpdateWindow();
	}
}
...
void actionFullscreenToggle() {
    if (state.properties->video.windowSize == P_VIDEO_SIZEFULLSCREEN) {
		if (state.windowedSize == P_VIDEO_SIZEX3) {
			actionWindowSizeTriple();
		}
		else {
