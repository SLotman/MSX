...
static HMENU menuCreateZoom(Properties* pProperties, Shortcuts* shortcuts)
{
	...
	sprintf(langBuffer, "%s      \t", langMenuZoomTriple());
	AppendMenu(hMenu, MF_STRING | (pProperties->video.windowSize == P_VIDEO_SIZEX3 ? MFS_CHECKED : 0), ID_SIZE_X3, langBuffer);
...
int menuCommand(Properties* pProperties, int command) 
{
	...
	case ID_SIZE_X3:                        actionWindowSizeTriple();       return 0;
