...
static int getZoom() {
	...
	// kludge for GDI
	if (pProperties->video.driver == P_VIDEO_DRVGDI) {
		if (pProperties->video.windowSize == P_VIDEO_SIZEX3) return 2;
	} else {
...
void themeSet(char* themeName, int forceMatch) {
    ...
    case P_VIDEO_SIZEX2:
        ...
        break;
	case P_VIDEO_SIZEX3:
		st.themePageActive = themeGetCurrentPage(st.themeList[st.themeIndex]->triple);
		break;
    ...
    if (pProperties->video.windowSize != P_VIDEO_SIZEFULLSCREEN) {
        ...
        if (pProperties->video.windowSize == P_VIDEO_SIZEX3) {
            ew = appConfigGetInt("screen.normal.width", 640+320);
            eh = appConfigGetInt("screen.normal.height", 480+240);
        }
    ...
    if (pProperties->video.windowSize != P_VIDEO_SIZEFULLSCREEN) {
		...
		if (pProperties->video.windowSize == P_VIDEO_SIZEX3) {
			st.hBitmap = CreateCompatibleBitmap(st.hdc, 640 + 320, 480 + 240);
		} else {
...
//        SetWindowRgn(st.hwnd, NULL, TRUE);
...

    {
        ...
        if (pProperties->video.windowSize == P_VIDEO_SIZEX3) {
            r.right = appConfigGetInt("screen.normal.width", 640+320);
            r.bottom = appConfigGetInt("screen.normal.height", 480+240);
        }
...
#define PRINT_RENDERING_TIME 0
		...
		if (pProperties->video.driver == P_VIDEO_DRVDIRECTX_D3D)
			...
			int param = 0;
			if (pProperties->video.windowSize == P_VIDEO_SIZEX2) param = 1;
			if (pProperties->video.windowSize == P_VIDEO_SIZEX3) param = 2;
