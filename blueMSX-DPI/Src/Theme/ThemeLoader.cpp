...
enum ThemeInfo { THEME_SMALL = 0, THEME_NORMAL = 1, THEME_FULLSCREEN = 2, THEME_TRIPLE = 3 };
...
extern "C" ThemeCollection* themeLoad(const char* themePath) 
{
	...
	themeCollection->triple			 = loadMainTheme(themeCollection, root, THEME_TRIPLE);
...
extern "C" ThemeCollection** createThemeList(ThemeCollection* defaultTheme)
{
    ...
    if (singleTheme != NULL) {
        ...
        if (themeCollection != NULL) {
			...
			if (themeCollection->triple == NULL)          themeCollection->normal =			 themeList[0]->triple;
